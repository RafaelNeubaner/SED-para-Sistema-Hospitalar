...
private:
public:
    int estado = 0; // estado do paciente
    ...
    double tmin;//tempo minimo que o paciente ficara no hospital
    double tat;//tempo de atendimento
    double tesp;//tempo de espera
    int mesout;//mes de saida
    int diaout;//dia de saida
    double horaout;//hora de saida
    int anoout;//ano de saida
    double horafila;//  hora de entrada na fila
    ...
    void calculateDateTime(double totalHours); // função que calcula a data e hora de saida do paciente
};

std::string converterHoras(double horasDecimais);//função que converte as horas decimais em horas normais
...
double horatotal(int ano, int mes, int dia, int hora); // função que calcula o tempo total em horas
